...
def index():
    ...
    popular_books = the_books.outerjoin(Log).group_by(Book.id).order_by(db.func.count(Log.id).desc())
    popular_users = User.query.outerjoin(Log).group_by(User.id).order_by(db.func.count(Log.id).desc())
    recently_comments = Comment.query.filter_by(deleted=0).order_by(Comment.edit_timestamp.desc())
